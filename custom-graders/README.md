...
- Use of environment variables as part of the grader is not prohibited but we do limit the ability to set environment variables as part of 'Dockerfile' as we do sanitize the Docker environment before running the graders for security purposes. Hence at runtime, its not to safe to rely on environment variables that you set in 'Dockerfile'.
